<?php
	...
	function generateRow($conn){
        ...
        while($row = $query->fetch_assoc()){
        	...
        	$candidateSql = "SELECT * FROM candidates WHERE position_id = '$id' ORDER BY lastname ASC";
        	$candidateQuery = $conn->query($candidateSql);

        	if ($candidateQuery->num_rows > 0) {
        		$hasData = true;
	    		...
	    		while($crow = $candidateQuery->fetch_assoc()){
        ...
        if (!$hasData) {
        	$contents .= '
        		<tr>
        			<td colspan="2" align="center">No results available yet. No votes have been cast.</td>
        		</tr>
        	';
        }
